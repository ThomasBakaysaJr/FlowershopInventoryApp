...
def test_undo_production_handles_legacy_pack_logs(mock_db):
    """A legacy PACK log (pre-collapse) is undone without touching inventory — PACK never deducted anything."""
    conn = sqlite3.connect(mock_db)
    cursor = conn.cursor()
    ...
    cursor.execute(
        "INSERT INTO production_goals (product_id, qty_ordered, qty_fulfilled, due_date) VALUES (?, 5, 1, '2026-02-14')",
        (p_id,),
    )
    g_id = cursor.lastrowid
    cursor.execute(
        "INSERT INTO production_logs (goal_id, product_id, action_type) VALUES (?, ?, 'PACK')",
        (g_id, p_id),
    )
    conn.commit()
    conn.close()
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT count_on_hand FROM inventory WHERE item_id = ?", (vase_id,))
    assert cursor.fetchone()[0] == 50  # no change — PACK never deducted
    cursor.execute("SELECT qty_fulfilled FROM production_goals WHERE goal_id = ?", (g_id,))
    assert cursor.fetchone()[0] == 0
    cursor.execute("SELECT COUNT(*) FROM production_logs WHERE goal_id = ?", (g_id,))
    assert cursor.fetchone()[0] == 0
...
def test_stock_rollover(mock_db):
    """Updating a recipe with rollover_stock=True carries over the stock count (catalog feature)."""
    conn = sqlite3.connect(mock_db)
    cursor = conn.cursor()
